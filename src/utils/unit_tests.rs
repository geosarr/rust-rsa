...
mod tests {
    ...
    fn test_euclid_algo() {
        ...
        for _ in 0..10000 {
            ...
            if num1 != num2 {
                assert_eq!(
                    BigInt::from_biguint(Plus, gcd),
                    coefs[&num1].clone() * int_num1.clone()
                        + coefs[&num2].clone() * int_num2.clone()
                );
            } else {
                assert_eq!(gcd, num1);
                assert_eq!(gcd, num2);
    ...
    fn test_encrypt_decrypt() {
        ...
        for _ in 0..10 {
            ...
            let vig_key_size = rng
                .gen_biguint_range(&one, &max_key_size)
                .to_string()
                .parse::<usize>()
                .unwrap();
            // Generate keys
